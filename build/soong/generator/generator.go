...
func init() {
	android.RegisterModuleType("xperia_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = xperiaExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = xperiaExpandVariables(ctx, dep_file)
	...
	cmd := xperiaExpandVariables(ctx, String(g.properties.Cmd))
